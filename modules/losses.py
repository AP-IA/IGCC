...
class ConvNextDistillDiffPruningLoss(nn.Module):
    ...
    def __init__(self, teacher_model, ratio_weight=10.0, distill_weight=0.5, keep_ratio=[0.9, 0.7, 0.5],
                 swin_token=False):
        super().__init__()
    ...
    def forward(self, inputs, outputs):
        ...
        # ratio loss
        ratio_loss = torch.tensor(0.0).to(inputs.device)
...
class DistillDiffPruningLoss_dynamic(torch.nn.Module):
    ...
    def __init__(self, teacher_model, ratio_weight=2.0, distill_weight=0.5, dynamic=False, pruning_loc=[3, 6, 9],
                 keep_ratio=[0.75, 0.5, 0.25], mse_token=False):
        super().__init__()
        self.teacher_model = teacher_model
